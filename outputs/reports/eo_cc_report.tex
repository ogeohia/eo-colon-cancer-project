...
\hypertarget{early-onset-colon-cancer-trends-analysis-ci5plus}{%
...
\href{https://github.com/ogeohia/eo-colon-cancer-project}{eo-colon-cancer-project}
...
\item
  ...
  males show \textasciitilde20\% higher incidence (IRR $\\approx$ 1.20)
\item
...
See \href{data/README.md}{\texttt{data/README.md}} for detailed data
...
\item
  \textbf{Age range:} 15--79 years (exclude ages 0--14, 80+, and
  unknown)
...
country-level (n=48 countries) vs.~stratum-level (n$\\approx$90,000
...
diagnostics (R̂, ESS, divergent transitions), MCMC sampling
...
\item
  \textbf{Total draws:} 8,000 (4 chains × 2,000 iterations)
\item
...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth,height=\textheight]{../cmdstan_run/gq_1664802/age_incidence_curve.png}
...
median region/country random effects). The curve exhibits characteristic
exponential increase with age, acceleratinh sharply after about
\textasciitilde50, consistent with somatic mutation accumulation models
and prolonged carcinogenic exposures.}
...
\item
  ...
  \item
    Over 10 years: \((1.012)^{10} \approx 1.13\) → 13\% cumulative
    increase
  \item
    Over 39 years (1978--2017): \((1.012)^{39} \approx 1.59\) → 59\%
    cumulative increase
  \end{itemize}
\end{itemize}
...
\item
  ...
  \item
    \textbf{Severity:} Observed far exceeds 90\% PI (\textgreater4\sigma
    deviation)
  \end{itemize}
\end{itemize}
...
\item
  ...
  \item
    May need \textbf{region × year interactions} to capture differential
    temporal trends
  \item
    Or \textbf{region × age interactions} to allow age patterns to vary
    by region
  \end{itemize}
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Allow \beta\_year to vary by country}
...
\item
  ...
  \tightlist
  \item
    Air pollution (PM2.5, NO$_2$) from satellite data
  \item
